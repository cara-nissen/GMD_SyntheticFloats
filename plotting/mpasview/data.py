...
class MPASOMap:
    ...
    def plot(
     ...
     #       axis = plt.gca()
     #   else: #CN
     #       print('axis defined in plot function')
     #       axis=axis
        ...
        if colorbar:
            ...
            #l1 = np.arange(-90,-45+1,1)
            #l2 = np.arange(-180,180+1,1)
            #lon,lat = np.meshgrid(l1,l2)
            #a1 = 250*np.ones_like(lon)
            #a1[:,2] = 280
            #fig=m.contourf(lon,lat,-999*np.ones_like(lon))
            
            cb = plt.colorbar(fig, ax=m,shrink=0.85,pad=0.01) # CN: added shrink, pad
